...
static void mavlink_test_tilt_angle_setpiont(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_TILT_ANGLE_SETPIONT >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_tilt_angle_setpiont_t packet_in = {
        963497464,45.0,73.0,101.0,129.0
    };
    mavlink_tilt_angle_setpiont_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_TILT_ANGLE_SETPIONT_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_TILT_ANGLE_SETPIONT_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_tilt_angle_setpiont_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_tilt_angle_setpiont_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_tilt_angle_setpiont_pack(system_id, component_id, &msg , packet1.time_boot_ms , packet1.tilt_fl , packet1.tilt_fr , packet1.tilt_rl , packet1.tilt_rr );
    mavlink_msg_tilt_angle_setpiont_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_tilt_angle_setpiont_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.time_boot_ms , packet1.tilt_fl , packet1.tilt_fr , packet1.tilt_rl , packet1.tilt_rr );
    mavlink_msg_tilt_angle_setpiont_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_tilt_angle_setpiont_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_tilt_angle_setpiont_send(MAVLINK_COMM_1 , packet1.time_boot_ms , packet1.tilt_fl , packet1.tilt_fr , packet1.tilt_rl , packet1.tilt_rr );
    mavlink_msg_tilt_angle_setpiont_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("TILT_ANGLE_SETPIONT") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_TILT_ANGLE_SETPIONT) != NULL);
#endif
}
...
static void mavlink_test_development(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_tilt_angle_setpiont(system_id, component_id, last_msg);
